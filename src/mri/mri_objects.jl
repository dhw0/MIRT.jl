...
function mri_objects_image_dirac2(data::RealArray, x::RealArray, y::RealArray,
    ...
    inputParams = [data[:,1:2] zeros(size(data,0),1) data[:,3]]
...
function mri_objects_case4(fov::AbstractVector{<:Real} ; unit::Symbol=:mm)
    ...
    cp = [0 0 0 0.4fov[1] fov[3] 1] # cyl3
    rp = [ # rect3
      -50 -50  0  40 40 40  1;
       50 -50 40  20 20 50  1;
       0  50 -40  30 30 60  1;
    ...
    return [(:cyl3, cp), (:rect3, rp), (:gauss3, gp)]
end
...
function mri_objects_test2()
    ...
    ntest = length(tests)
    pl = Matrix{Plots.Plot}(undef, ntest, ntest)
    ...
    #   @show round.(maximum.([i2, abs.(f2), abs.(s2)]), digits=2)
        pl[1,i] = jim(i2, title = "$(test[1])")
        pl[2,i] = jim(abs.(s2), title = "|fft|")
        pl[3,i] = jim(abs.(f2), title = "|kspace|")
    end
    plot(pl...)
end
...
function mri_objects_test3()
    ...
    for (i,test) in enumerate(tests)
        ...
        pl[1,i] = jim(i3, title = "$(test[1])")
        pl[2,i] = jim(abs.(s3), title = "fft")
        pl[3,i] = jim(abs.(f3), title = "kspace")
    end
    plot(pl...)
end
